...
class HG(object):
    ...
    def heter_cam_normalization(self, k1_graph):
        for i in range(len(k1_graph)):
            ...
            for c in tag_c_set:
                ...
                w = len(w) / len(cd_c) * w / np.sum(w)  # 1/len(w),考虑在计算游走概率的时候加上迁移时空概率
    ...
    def old_delta_propagation(self, ks=2, kd=4, k2=11, opt=None):
        print("gause kernel")
        delta = 1.
        ...
        if self.general_graph:
            ...
            st_graph = self.kneighbors(st_scores, kd)
        if self.homo_ap:
            print("----homo-ap----")
