...
def get_query_embeddings(gnn_model, len_g, lengq):
    print('Loading embeddings from trained GraphSAGE model.')
    nodes = np.arange(len_g, lengq).tolist()
    b_sz = 500
...
def get_gnn_embeddings(gnn_model, len_nodes):
    print('Loading embeddings from trained GraphSAGE model.')
    # features = np.zeros((len(getattr(dataCenter, ds+'_labels')), gnn_model.out_size))
    nodes = np.arange(len_nodes).tolist()
    b_sz = 500
    batches = math.ceil(len(nodes) / b_sz)
    embs = []
    for index in range(batches):
