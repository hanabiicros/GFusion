...
def get_data(opt, source, target, iters=200):
    ...
    test_graph_transformer = T.Compose([
        T.Resize((height, width), interpolation=3),
        T.ColorJitter(brightness=0.2, saturation=0.0, hue=0.0),
        T.ToTensor(),
        normalizer,
    ])
    ...
    return s_pids, s_train_loader, t_train_real,t_traintest_loader, s_query_loader, s_gallery_loader, s_query, s_gallery
...
def main_worker(args):
    ...
    s_pids, s_train_loader, t_train_real,t_feat_loader,s_query_loader, s_gallery_loader, s_query, s_gallery = \
        get_data(args, args.source, args.target, iters)

    t_pids, t_train_loader, s_train_real,s_feat_loader,t_query_loader, t_gallery_loader, t_query, t_gallery = \
    ...
    if args.resume:
        ...
        model.load_state_dict(checkpoint['backbone_dict'], strict=False)
    ...
    if args.target_training:
        evaluator.transfer(t_feat_loader,t_train_real,args.eval_dir)
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_training', action='store_true', help="get info on target training only")
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default='/hdd/sdb/lsc/dataset_person')
